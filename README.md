...
Většinu dne bude M v režimu Stop2 s RTC. Tento režim se vyznačuje velmi nízkou spotřebou a narozdíl od režimu StandBy s RTC má mimo jiné možnost udržet logické úrovně a nastavení pinů. M je automaticky taktovaný externím krystalem LSE, umístěným na LoRa-E5 mini, na 32 kHz. Když ale RTC hodiny zavelí že je čas na práci, tak se M přepne do režimu LPRun (Low Power Run). V tomto režimu je taktovaný interním krystalem LSI na 1 MHz. V průběhu LoRa přenosu (Radio TX / RX) se CPU přepne do Sleep režimu. M je automaticky taktovaný externím krystalem HSE, umístěným na LoRa-E5 mini, na 32 MHz.

U Mx to bude po většinu dne velmi podobné. Bude se nacházet v úsporném režimu Stop bez RTC, ze stejných důvodů jako M. Mx bude ale probouzeno přes M, přesněji přes LPUART. Tudíž nepotřebuje RTC hodiny. Po probuzení se Mx přepne do režimu LPRun (LSI, 131 kHz).

Pozor na plovoucí piny! Pokud piny nejsou používány, tak se musí kvůli spotřebě nastavit do analogového režimu bez pull rezistoru. Kvůli spotřebě je také potřeba odpojit programátor ST-Link, před přepnutím do  úsporného režimu nastavit externí RF switch na logickou nulu a použít LP rádio nejen softwarově, ale i výstup LP radio.

Před odpojením napájení VCC různých částí systému je potřeba piny (SCK, DT, PH, EN, DI, DE, RE, RO) přepsat na logickou nulu, pro zamezení napájení přes piny tomu neurčené (leakage current). Dále budou vypnuty periferie (I²C, UART, ADC), protože i když se přes ně nic neposílá, tak jim běží hodinový signál, který plýtvá energii. Po odpojení VCC je potom třeba nastavit všechny piny, včetně těch pro teď už vypnuté periferie, do analogového režimu bez pull rezistoru.
